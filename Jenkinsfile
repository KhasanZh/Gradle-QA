...
def test(tag) {
        stage("Download separate repository"){
           checkout scm: [
                        ...
                        userRemoteConfigs: [[
                            credentialsId: '7536738a-1a9a-4250-82da-6553e4db84ce',
                            url: 'git@gitlab.com:threadqacourses/threadqagradletests.git'
                        ]]
                      ]
        }
    withEnv(["tag=${tag}"]) {
...
def runTests(tag) {
    ...
    if (runFailedTests()) {
        ...
        ''')
            ...
            chmod +x gradlew
            ./gradlew clean myTags -x test -DcustomTags=$tag -Dhost=${SELENOID_HOST}
            ''')
        }
    }
}
